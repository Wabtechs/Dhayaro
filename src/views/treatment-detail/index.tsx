...
import { treatmentEditSchema, toTreatmentPayload, TREATMENT_STATUSES, type TreatmentEditValues } from '@/lib/schemas'
...
export default function TreatmentDetailPage() {
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Skeleton className="h-px w-full" />
                <div className="flex items-start gap-3"><Skeleton className="h-4 w-4 rounded" /><div className="space-y-1"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-32" /></div></div>
                <Skeleton className="h-px w-full" />
                <div className="flex items-start gap-3"><Skeleton className="h-4 w-4 rounded" /><div className="space-y-1"><Skeleton className="h-3 w-20" /><Skeleton className="h-4 w-28" /></div></div>
                <div className="flex items-start gap-3"><Skeleton className="h-4 w-4 rounded" /><div className="space-y-1"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-24" /></div></div>
              </CardContent>
            </Card>
          </div>
        </div>
  ...
  const openEditDialog = () => {
    editForm.reset({
      ...
      status: (TREATMENT_STATUSES as readonly string[]).includes(status) ? (status as TreatmentEditValues['status']) : 'PRESCRIBED',
      startDate: startDate.slice(0, 10),
      endDate: endDate.slice(0, 10),
      notes: String(c.notes ?? notes),
      outcome: String(c.outcome ?? outcome),
    })
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <Input id="endDate" type="date" {...editForm.register('endDate')} />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="notes">Notes</Label>
                <Textarea id="notes" rows={2} {...editForm.register('notes')} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="outcome">Issue / Résultat</Label>
                <Textarea id="outcome" rows={2} {...editForm.register('outcome')} />
              </div>
            </div>
            <DialogFooter>
